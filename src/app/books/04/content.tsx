...
const themeColor = '#f29998'
const themeSecondaryColor = '#038579'
const themeTertiaryColor = '#26161c'
  ...
  return useMemo(() => {
    return {
      ...
      backgroundOverlay2: css`
        ...
        background-color: rgba(182, 173, 171, 0.85);
...
const MainVideo = ({ width, height, ...props }: MainVideoProps) => {
  return (
    <BackgroundVideo
      ...
      src={{
        default: {
          url: '/videos/book04-1920x1080.mp4',
          type: 'video/mp4',
        },
      }}
      poster={{
        default: '/videos/book04-1920x1080.jpg',
      }}
    />
  )
}

export default function Book04Content() {
      ...
      <layout.Inner css={styles.containerInner}>
        <layout.Inner1 css={styles.inner1}>
          ...
          <layout.ContentTitle css={styles.contentTitle}>
            {books.vol04.titleMultiline.map((x, i) => <span key={i}>{x}</span>)}
          </layout.ContentTitle>
